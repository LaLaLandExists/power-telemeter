...
This paper presents the design, implementation, and evaluation of a wireless power telemetry system capable of monitoring and remotely controlling up to eight alternating-current (AC) loads in real time without dependence on cloud infrastructure or cellular networks. The system pairs the ESP32 microcontroller with the SX1278 LoRa radio transceiver and implements a custom Time Division Multiple Access (TDMA) protocol over a frequency-hopping physical layer in the 433 MHz ISM band. A gateway node hosts a web-based single-page application (SPA) served from on-chip flash and exposes a REST and WebSocket API over Wi-Fi. Up to eight sensor nodes, each interfacing a PZEM-004T v3 power meter and an AC relay, transmit six electrical measurements per superframe (3-second period) and respond to relay commands within one scheduling cycle. Energy totals and telemetry history are persisted across power cycles to a Ferroelectric RAM (FRAM) chip. An optional AES-128 CTR encryption layer protects command integrity in security-sensitive deployments. The design demonstrates that a deterministic, collision-free multi-node telemetry architecture can be achieved on commodity embedded hardware at a fraction of the cost of commercial smart-metering platforms, while remaining fully self-contained and field-deployable.
...
6. Optional relay-command authentication via symmetric encryption, provisionable without a key-management server.

The contribution of this work is not the invention of any individual technique — LoRa, TDMA, FRAM, and AES-CTR are all well-established — but their integration into a coherent, production-grade embedded system whose design trade-offs are explicitly motivated by the constraints of field deployment on commodity hardware.

---

## 2. Background and Literature Review

### 2.1 Wireless Sensor Networks for Energy Monitoring
...
### 2.3 TDMA MAC Protocols for Wireless Sensor Networks
...
### 2.4 PZEM-004T v3 for Embedded Energy Metering
...
### 2.5 Real-Time Operating Systems on ESP32
...
### 2.6 FRAM for High-Endurance Data Logging
...
### 2.7 AES-128 CTR Encryption for Embedded IoT

Counter (CTR) mode converts a block cipher into a stream cipher, producing ciphertext of identical length to the plaintext — critical for the fixed-length radio packets used in this system. Unlike CBC mode, CTR mode is immune to padding-oracle attacks and parallelisable [19]. The mbedTLS library, bundled with ESP-IDF, provides a hardware-accelerated AES-128 implementation on the ESP32 that completes a 32-byte encryption in under 5 µs [20], well within the 50 ms downlink window.

Hossain et al. [21] survey lightweight cryptographic schemes for resource-constrained IoT nodes and identify AES-128 CTR as the best balance of security and computational overhead for microcontroller-class devices when a hardware RNG is available for nonce generation. In this design, the nonce is derived deterministically from the superframe counter and slot ID rather than a random number, which is safe in CTR mode provided the key is not reused across sessions — a guarantee provided by the RFID provisioning flow, which generates a fresh key per gateway boot.
...
6. **No physical-layer authentication:** Unencrypted LoRa broadcasts are trivially replayable; an attacker within radio range can toggle any relay by replaying a previously captured command.

This project directly addresses all six limitations through a unified embedded system design.
...
### 5.2 Physical Layer — LoRa at SF6
...
### 5.3 MAC Layer — Custom TDMA Protocol
...
The 1520 ms idle window following the contention phase serves three purposes: it provides time for gateway maintenance tasks (node eviction, deferred FRAM writes dispatched over a FreeRTOS queue); it reduces the ISM-band duty cycle to comply with European ETSI EN 300 220 1% limits [24]; and it leaves room for future superframe extension if additional protocol overhead is required.

#### 5.3.2 Beacon and Time Synchronisation
...
### 5.4 Sensor Subsystem — PZEM-004T v3

Each sensor node interfaces a PZEM-004T v3 power meter on ESP32 UART2 (RX=GPIO22, TX=GPIO23) using the Modbus RTU protocol at 9600 baud. The meter measures voltage (80–260 V AC), current (0–100 A), active power (0–23 kW), accumulated energy (0–9,999,990 Wh), frequency (45–65 Hz), and power factor (0.00–1.00) at the resolutions listed in Section 2.4. An AC relay (active-HIGH on GPIO25) controlled by the ESP32 allows the dashboard to energise or de-energise the monitored load.
...
All packet structures are `#pragma pack(1)` to eliminate compiler-inserted padding, ensuring byte-exact layout between the node's transmit buffer and the gateway's receive buffer across any GCC version or target architecture [22]. Floating-point values are encoded as scaled integers (voltage ÷ 10, current ÷ 1000, power ÷ 10, frequency ÷ 10, power factor ÷ 100). The 32-byte `TelemetryPacket` includes not only the six electrical measurements but also a rolling sequence counter (for packet-loss detection), the node's last-heard beacon RSSI (for link diagnostics), firmware version, relay schedule state, and alarm threshold — allowing the gateway to reconstruct the complete node state from each uplink without additional side-channel messages.

A 2-byte `DlHeader` (pktType, nodeId) prefixes every downlink packet. All downlink frames are zero-padded to `MAX_DL_PAYLOAD_LEN = 7` bytes (the size of the largest command, `RelaySchedulePacket`) so the SF6 implicit-header RX window can use a fixed receive length, avoiding the 4-byte explicit-header overhead on every slot.

### 5.6 Application Layer — Web Dashboard
...
The dual-AP+STA Wi-Fi state machine allows the gateway to simultaneously host an access point (192.168.4.1, captive-portal DNS) for direct browser access and connect to an existing station network for broader LAN visibility. A DNS catch-all on the AP redirects all hostnames to the gateway, supporting the captive-portal detection flows of Android, iOS, Windows, and Firefox OS [28].

### 5.7 Data Persistence — FRAM
...
### 5.8 Optional Encryption Layer

When compiled with the `PKT_ENCRYPTION` build flag, `pkt_crypto.h` wraps all radio TX/RX with AES-128 CTR encryption via mbedTLS. A per-packet nonce is constructed deterministically from the superframe counter and slot ID:

```
nonce[16] = [ sfCount_lo | sfCount_hi | slotId | dir | 0x00 × 12 ]
```

Both sides derive the nonce independently from shared protocol state, requiring no nonce transmission overhead. Mixing `sfCount` and `slotId` ensures every packet in a superframe uses a unique keystream block, preventing keystream reuse. The two-byte `sfCount` field in the `BeaconPacket` is transmitted plaintext so nodes can construct the nonce before decryption, breaking an otherwise circular dependency.

Symmetric key distribution uses PN532 I²C NFC readers: the gateway auto-generates a 128-bit key on first boot, stores it in NVS, and writes it to a MIFARE Classic card via the web dashboard. Each node blinks red until a provisioned card is presented to its PN532, then stores the key and reboots into normal operation. Physical proximity (~4 cm NFC range) serves as the trust boundary, eliminating the need for a key-management server while meeting the security requirements of an on-site deployment.

---

## 6. Implementation

The firmware is developed in C++11 under the Arduino framework on PlatformIO, targeting the `esp32dev` board profile. Six build environments cover all combinations of role (gateway / node), encryption (on / off), and PZEM mode (hardware / simulated):

| Environment          | Role    | Encryption     | PZEM      |
| -------------------- | ------- | -------------- | --------- |
| `gateway`          | Gateway | off            | n/a       |
| `gateway_enc`      | Gateway | PKT_ENCRYPTION | n/a       |
| `node`             | Node    | off            | hardware  |
| `node_enc`         | Node    | PKT_ENCRYPTION | hardware  |
| `node_nettest`     | Node    | off            | simulated |
| `node_enc_nettest` | Node    | PKT_ENCRYPTION | simulated |
...
- **Affordability:** The complete gateway hardware costs approximately ₱1,200–₱1,500 (USD 20–25) and each sensor node approximately ₱800–₱1,000 (USD 14–17), versus ₱8,000–₱15,000 for equivalent commercial multi-circuit smart-meter gateways.
...
- **Academic contribution:** The integration of a custom TDMA MAC, frequency-hopping physical layer, dual-clock schedule reliability, network-epoch slot invalidation, deferred cross-slot FRAM restore, and AES-CTR command authentication into a single embedded system on commodity hardware represents a practical reference design for future multi-node LoRa telemetry projects.

---

## 8. Conclusion

This concept paper has presented the motivation, literature basis, design objectives, and architecture of an ESP32-based LoRa TDMA power telemetry system. The system synthesises well-established techniques — TDMA MAC scheduling [11], LoRa CSS physical layer [3], frequency-hopping interference mitigation [12], PZEM-004T v3 AC metering [13], FRAM persistence [18], AES-128 CTR encryption [19], and FreeRTOS dual-core task pinning [17] — into a coherent, production-grade embedded platform that operates entirely without cloud infrastructure.
...
[11] I. Demirkol, C. Ersoy, and F. Alagöz, "MAC protocols for wireless sensor networks: a survey," *IEEE Communications Magazine*, vol. 44, no. 4, pp. 115–121, Apr. 2006. doi:10.1109/MCOM.2006.1632658
...
[19] M. Bellare, J. Killian, and P. Rogaway, "The Security of Cipher Block Chaining," in *Advances in Cryptology — CRYPTO 1994*, LNCS vol. 839. Springer, 1994, pp. 341–358. doi:10.1007/3-540-48658-5_32

[20] Espressif Systems, "ESP32 Hardware Accelerated AES," ESP-IDF Programming Guide v5.2, 2023. [Online]. Available: https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/aes.html

[21] M. J. Hossain, M. A. Fotouhi, and R. Hasan, "Towards an analysis of security issues, challenges, and open problems in the Internet of Things," in *Proc. IEEE World Congress on Services (SERVICES)*, 2015. doi:10.1109/SERVICES.2015.12

[22] B. Kernighan and R. Pike, *The Practice of Programming*. Addison-Wesley, 1999, ch. 2.

[23] Semtech Corporation, *SX1276/77/78/79 Datasheet: 137 MHz to 1020 MHz Low Power Long Range Transceiver*, Rev. 7, Semtech, 2020.

[24] European Telecommunications Standards Institute, *ETSI EN 300 220-1 V3.1.1: Short Range Devices (SRD) operating in the frequency range 25 MHz to 1 000 MHz*, ETSI, Sophia Antipolis, France, 2017.

[25] S. Sesia, I. Toufik, and M. Baker, *LTE — The UMTS Long Term Evolution: From Theory to Practice*, 2nd ed. Wiley, 2011, ch. 4.

[26] C. Buratti, A. Conti, D. Dardari, and R. Verdone, "An Overview on Wireless Sensor Networks Technology and Evolution," *Sensors*, vol. 9, no. 9, pp. 6869–6896, Sep. 2009. doi:10.3390/s90906869

[27] I. Fette and A. Melnikov, "The WebSocket Protocol," IETF RFC 6455, Dec. 2011. doi:10.17487/RFC6455

[28] Wi-Fi Alliance, *Captive Portal Technical Specification v1.1*, Wi-Fi Alliance, 2021.
