...
static QueueHandle_t s_logLineQueue = nullptr;
static char          s_lineAcc[LOG_LINE_MAX];
static uint8_t       s_lineAccLen = 0;
...
static void logDrainTask(void* /*params*/) {
  ...
  while (true) {
    ...
    if (count > 0) {
      ...
      // Tee complete lines to the relay queue for WebSocket broadcast
      for (uint16_t k = 0; k < count; k++) {
        char c = (char)buf[k];
        if (c == '\n' || s_lineAccLen >= LOG_LINE_MAX - 1) {
          if (s_lineAccLen > 0) {
            s_lineAcc[s_lineAccLen] = '\0';
            LogLine_t ll;
            memcpy(ll.data, s_lineAcc, s_lineAccLen + 1);
            xQueueSend(s_logLineQueue, &ll, 0);  // non-blocking, drop if full
          }
          s_lineAccLen = 0;
        } else {
          s_lineAcc[s_lineAccLen++] = c;
        }
      }
    } else {
...
bool logLineDequeue(char *buf, size_t maxLen) {
  if (!s_logLineQueue) return false;
  LogLine_t ll;
  if (xQueueReceive(s_logLineQueue, &ll, 0) != pdTRUE) return false;
  strncpy(buf, ll.data, maxLen - 1);
  buf[maxLen - 1] = '\0';
  return true;
}
