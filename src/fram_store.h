...
/**
 * Queue a deferred FRAM restore of accumEnergy and history[] for one slot.
 * The FRAM task verifies the stored UID matches g_nodes[nodeIdx].deviceUID
 * before applying data, so it is safe to call on every new slot allocation.
 * Non-blocking: the request is dropped silently if the queue is full.
